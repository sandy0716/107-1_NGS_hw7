...
# Last date modified: Thu Jul 26 20:33:43 2018
# ---------------------------- #
...
coldata <- coldata[,c("condition","type")] # Only choose the two columns for "condition" and "type"
...
rownames(coldata) <- sub("fb", "", rownames(coldata))  # Replace "fb" with ""
cts <- cts[, rownames(coldata)]  # Put Rownames into count matrix cts

all(rownames(coldata) %in% colnames(cts)) #的check 是不是所有coldata的rownames都有在cts的colnames
...
dds$condition
dds$condition <- relevel(dds$condition, ref = "untreated") #把untreated當成reference
dds <- DESeq(dds)#執行DESeq
res <- results(dds)
res
...
# Check the shrinkage estimation of dispersion
...
resSig <- subset(res[order(res$padj),], padj < 0.05)#把Pvalue<0.05的排序好篩出來 throw it into subset resSig
resSig
